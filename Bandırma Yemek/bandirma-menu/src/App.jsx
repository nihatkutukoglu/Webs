...
// Firebase Modülleri
import { 
  collection, addDoc, deleteDoc, doc, onSnapshot, query, orderBy, serverTimestamp, updateDoc, increment 
} from 'firebase/firestore';
import {  getAuth, signInAnonymously, onAuthStateChanged, signInWithEmailAndPassword, signOut
} from 'firebase/auth';
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-24 md:pb-10">
      ...
      <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-sm shadow-sm border-b border-slate-200 safe-top">
        <div className="max-w-md mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <button 
            onClick={() => isAdmin ? signOut(auth) : setShowLoginModal(true)}
...
// --- Kampanya Destekli Mekan Ekleme Formu (Sadece Link) ---
function AddVenueModal({ onClose, categories, appId, db }) {
  const [formData, setFormData] = useState({
    name: '', category: 'Restoran', address: '', phone: '',
    menuUrl: '', logoUrl: '', 
    campaignText: '', campaignColor: '#ef4444'
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Kaydet
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'venues'), finalData);
      onClose();
    } catch (error) {
      console.error(error);
            ...
            <div className={activeTab === 'details' ? 'block' : 'hidden'}>
               ...
               <div className="mt-4 space-y-4">
                 ...
                 <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                    <span className="text-xs font-bold text-slate-700 block mb-2">Menü Görseli (Link - Zorunlu)</span>
                    <input 
                      placeholder="https://ornek.com/menu.jpg" 
                      className="w-full border-slate-200 rounded-lg p-2 text-sm focus:ring-2 focus:ring-orange-500 outline-none" 
                      value={formData.menuUrl} 
                      onChange={e => setFormData({...formData, menuUrl: e.target.value})} 
                    />
                    <p className="text-[10px] text-slate-400 mt-1">İnternetten bulduğunuz bir resmin adresini yapıştırın.</p>
                 </div>

                 {/* Logo Görseli Seçimi */}
                 <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                    <span className="text-xs font-bold text-slate-700 block mb-2">Logo (İsteğe Bağlı)</span>
                    <input 
                        placeholder="https://ornek.com/logo.jpg" 
                        className="w-full border-slate-200 rounded-lg p-2 text-sm focus:ring-2 focus:ring-slate-500 outline-none" 
